...
pub fn emit_x86(lowered: &LoweredModule) -> Result<MachineCode> {
...
impl CodegenCtx {
    fn new(
        ...
        Self {
            ...
            free_regs: vec![Reg::EAX, Reg::EBX, Reg::ECX, Reg::EDX, Reg::ESI, Reg::EDI],
            uses,
            phi_map,
        ...
        for instr in &block.instrs {
            match instr {
                IrInstr::LoadConstInt { dst, value, .. } => {
                    ...
                    emit_mov_reg_imm(&mut self.code, reg, *value as i32);
                }
                ...
                IrInstr::LoadConstI32 { dst, value } => {
                    let reg = self.ensure_reg(*dst)?;
                    emit_mov_reg_imm(&mut self.code, reg, *value);
                }
                ...
                IrInstr::Cmp {
                    ...
                    let kind = match cond {
                        ...
                        _ => return Err(anyhow!("unsupported cmp {:?}", cond)),
                    };
                ...
                IrInstr::Load { dst, ptr, .. } => {
                    let dst_reg = self.ensure_reg(*dst)?;
                    let src_reg = self.ensure_reg(*ptr)?;
                    emit_mov_reg_rm(&mut self.code, dst_reg, src_reg);
                    self.consume(*ptr);
                }
                IrInstr::Store { src, ptr, .. } => {
                    let src_reg = self.ensure_reg(*src)?;
                    let dst_reg = self.ensure_reg(*ptr)?;
                    emit_mov_rm_reg(&mut self.code, dst_reg, src_reg);
                    self.consume(*src);
                    self.consume(*ptr);
                }
                IrInstr::PrintStr { sid } => {
                    let len = strings
                        ...
                        .map(|s| s.len() as u32)
                        .unwrap_or(0);
                    emit_mov_eax(&mut self.code, 4); // sys_write
                    emit_mov_ebx(&mut self.code, 1); // fd=stdout
                    emit_mov_ecx_placeholder(&mut self.code, *sid, &mut self.string_patches);
                    emit_mov_edx(&mut self.code, len);
                    emit_int80(&mut self.code);
                }
                IrInstr::Phi { dst, .. } => {
                    self.ensure_reg(*dst)?;
                }
                _ => return Err(anyhow!("unsupported ir instr in x86 backend: {:?}", instr)),
            }
        }
        Ok(())
    }
    ...
    fn emit_terminator(&mut self, block_id: u32, term: &IrTerm) -> Result<()> {
        match term {
            ...
            _ => return Err(anyhow!("unsupported terminator {:?}", term)),
        }
        Ok(())
    }
    ...
    fn emit_exit(&mut self) {
        emit_mov_eax(&mut self.code, 1); // sys_exit
        emit_mov_ebx(&mut self.code, 0);
        emit_int80(&mut self.code);
    }
...
fn emit_mov_reg_imm(code: &mut Vec<u8>, reg: Reg, value: i32) {
...
fn emit_mov_reg_rm(code: &mut Vec<u8>, dst: Reg, src: Reg) {
    code.push(0x8B);
    code.push(modrm_byte(dst, src));
}

fn emit_mov_rm_reg(code: &mut Vec<u8>, dst: Reg, src: Reg) {
    code.push(0x89);
    code.push(modrm_byte(src, dst));
}
...
fn emit_sete_reg(code: &mut Vec<u8>, reg: Reg) {
    code.extend_from_slice(&[0x0F, 0x94, 0xC0 | reg.low_bits()]);
}

fn emit_setne_reg(code: &mut Vec<u8>, reg: Reg) {
    code.extend_from_slice(&[0x0F, 0x95, 0xC0 | reg.low_bits()]);
}

fn emit_setl_reg(code: &mut Vec<u8>, reg: Reg) {
    code.extend_from_slice(&[0x0F, 0x9C, 0xC0 | reg.low_bits()]);
}

fn emit_setle_reg(code: &mut Vec<u8>, reg: Reg) {
    code.extend_from_slice(&[0x0F, 0x9E, 0xC0 | reg.low_bits()]);
}

fn emit_setg_reg(code: &mut Vec<u8>, reg: Reg) {
    code.extend_from_slice(&[0x0F, 0x9F, 0xC0 | reg.low_bits()]);
}

fn emit_setge_reg(code: &mut Vec<u8>, reg: Reg) {
    code.extend_from_slice(&[0x0F, 0x9D, 0xC0 | reg.low_bits()]);
}

fn emit_movzx_reg8(code: &mut Vec<u8>, reg: Reg) {
    code.extend_from_slice(&[0x0F, 0xB6, 0xC0 | reg.low_bits()]);
}

fn emit_mov_eax(code: &mut Vec<u8>, imm: u32) {
    code.push(0xB8);
    code.extend_from_slice(&imm.to_le_bytes());
}

fn emit_mov_ebx(code: &mut Vec<u8>, imm: u32) {
    code.push(0xBB);
    code.extend_from_slice(&imm.to_le_bytes());
}

fn emit_mov_ecx_placeholder(code: &mut Vec<u8>, string_id: u32, patches: &mut Vec<Patch>) {
    code.push(0xB9);
    let offset = code.len();
    code.extend_from_slice(&0u32.to_le_bytes());
    patches.push(Patch { offset, string_id });
...
fn emit_mov_edx(code: &mut Vec<u8>, imm: u32) {
    code.push(0xBA);
    code.extend_from_slice(&imm.to_le_bytes());
}
