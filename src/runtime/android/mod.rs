...
pub fn builtin_android_app_run(
...
pub fn builtin_android_permissions_request(
    ...
    if args.len() < 2 {
        return Err(RuntimeError::new("request expects permissions module and permission name"));
    }
...
pub fn builtin_android_permissions_is_granted(
    ...
    if args.len() < 2 {
        return Err(RuntimeError::new("is_granted expects permissions module and permission name"));
    }
    ...
    let android_runtime = get_android_runtime(interp)?;
    let granted = android_runtime.jni_bridge.is_permission_granted(permission)?;
...
pub fn builtin_android_intent_send(
    ...
    if args.len() < 3 {
        return Err(RuntimeError::new("send expects intent module, action and extras"));
    }
...
pub fn builtin_android_storage_get_internal_path(
    ...
    if args.len() < 1 {
        return Err(RuntimeError::new("get_internal_path expects storage module"));
    }
...
pub fn builtin_android_storage_get_external_path(
    ...
    if args.len() < 1 {
        return Err(RuntimeError::new("get_external_path expects storage module"));
    }
...
pub fn create_android_module() -> Value {
    ...
    activity_module.insert("on_create".to_string(), Value::String("lifecycle_method".to_string()));
    activity_module.insert("on_start".to_string(), Value::String("lifecycle_method".to_string()));
    activity_module.insert("on_resume".to_string(), Value::String("lifecycle_method".to_string()));
    activity_module.insert("on_pause".to_string(), Value::String("lifecycle_method".to_string()));
    activity_module.insert("on_stop".to_string(), Value::String("lifecycle_method".to_string()));
    activity_module.insert("on_destroy".to_string(), Value::String("lifecycle_method".to_string()));
    android_module.insert("Activity".to_string(), Value::Module(activity_module));
    
    // Context type
    let mut context_module = HashMap::new();
    context_module.insert("show_toast".to_string(), Value::Builtin(builtin_android_context_show_toast));
    context_module.insert("set_view".to_string(), Value::Builtin(builtin_android_context_set_view));
    ...
    permissions_module.insert("request".to_string(), Value::Builtin(builtin_android_permissions_request));
    permissions_module.insert("is_granted".to_string(), Value::Builtin(builtin_android_permissions_is_granted));
    android_module.insert("permissions".to_string(), Value::Module(permissions_module));
    
    // intent sub-module
    let mut intent_module = HashMap::new();
    intent_module.insert("send".to_string(), Value::Builtin(builtin_android_intent_send));
    ...
    service_module.insert("start".to_string(), Value::String("placeholder".to_string()));
    android_module.insert("service".to_string(), Value::Module(service_module));
    
    // storage sub-module
    let mut storage_module = HashMap::new();
    storage_module.insert("get_internal_path".to_string(), Value::Builtin(builtin_android_storage_get_internal_path));
    storage_module.insert("get_external_path".to_string(), Value::Builtin(builtin_android_storage_get_external_path));
...
pub fn on_permission_result(permission: &str, granted: bool) {
    println!("[ANDROID-RUNTIME] Permission result: {} -> {}", permission, granted);
}

pub fn on_intent_received(action: &str, extras: &str) {
    println!("[ANDROID-RUNTIME] Intent received: {} with extras: {}", action, extras);
}
